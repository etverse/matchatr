...
rate   <- 0.08 * exp(beta_x * rbinom(n, 1, 0.4) + 0.4 * rnorm(n))
cohort <- data.frame(
  id         = seq_len(n),
  t          = pmin(rexp(n, rate), 5),
  d          = as.integer(rexp(n, rate) <= 5),
  exposure   = rbinom(n, 1, 0.4),
  confounder = rnorm(n)
)
# reproduce rate with final covariates
set.seed(77)
cohort$exposure   <- rbinom(n, 1, 0.4)
cohort$confounder <- rnorm(n)
rate <- 0.08 * exp(beta_x * cohort$exposure + 0.4 * cohort$confounder)
cohort$t <- pmin(rexp(n, rate), 5)
cohort$d <- as.integer(rexp(n, rate) <= 5)
```
...
```{r}
# Classical NCC (clogit, matched analysis)
