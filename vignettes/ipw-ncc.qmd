...
-   **`ipw_weight`** — the @samuelsen1997pseudolikelihood inverse inclusion
    probability 1/π_j for each sampled control; cases get weight 1. The inclusion
...
[@saarela2008nested]. The classical conditional analysis ties each control to its case's failure time,
...
reused for a *secondary* one [@stoer2012comparison]. `reuse_ncc_endpoint()` keeps the controls' primary
...
with a robust sandwich [@kang2017efficient].
...
difference [@borgan1997estimation]. Unlike a ratio it can be negative (a protective exposure lowers the
...
`contrast()` reports a single time-*constant* excess hazard (the Lin-Ying model;
@lin1994semiparametric).
To let the effect vary with time, `excess_risk()` returns the **cumulative**
excess hazard $B_j(t) = \int_0^t \beta_j(s)\,ds$ — the additive analogue of
`absolute_risk()`. A straight line means a constant rate difference; curvature
means a time-varying one.

```{r}
er <- excess_risk(fit_add, times = 1:5)
er
```
...
#| fig-height: 4
#| fig-alt: >
#|   Cumulative excess hazard of the exposure over follow-up estimated from the
#|   IPW nested case-control additive-hazards model, with a pointwise 95%
#|   confidence band. The curve rises roughly linearly, indicating an
#|   approximately constant excess rate.
ex <- er$estimates[er$estimates$term == "exposure", ]
plot(ex$time, ex$estimate, type = "b", pch = 19, col = "#8e44ad",
     ylim = range(ex$ci_lower, ex$ci_upper),
     xlab = "time", ylab = expression(hat(B)[x](t)),
     main = "IPW NCC cumulative excess hazard")
lines(ex$time, ex$ci_lower, lty = 2, col = "#8e44ad")
lines(ex$time, ex$ci_upper, lty = 2, col = "#8e44ad")
