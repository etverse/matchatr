...
ratio for a binary exposure `x`, then draw a 1:3 risk-set NCC sample from it.
...
# Incidence-density sampling: for each case, draw 3 controls from those still at
# risk at its failure time (t >= t_case), the case excluded.
m <- 3
cases <- cohort[cohort$d == 1, ]
cases <- cases[order(cases$t), ]
sets <- lapply(seq_len(nrow(cases)), function(k) {
  tc <- cases$t[k]; cid <- cases$id[k]
  elig <- cohort$id[cohort$t >= tc & cohort$id != cid]
  take <- elig[sample.int(length(elig), m)]
  rows <- cohort[cohort$id %in% c(cid, take), ]
  rows$case <- as.integer(rows$id == cid)
  rows$set  <- k
  rows$risk_time <- tc
  rows
})
ncc <- do.call(rbind, sets)
head(ncc)
```
